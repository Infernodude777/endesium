...
public class ProductionVoidStalkerEntity extends Monster implements GeoEntity {
	...
	protected void registerGoals() {
		...
		// The custom state machine handles combat positioning; these vanilla goals
		// keep the creature visibly active when it has no target.
	...
	private void updateBehavior() {
		...
		switch (state) {
			...
			case AWARE -> {
				if (stateTicks >= AWARE_TICKS) {
					enterState(los ? State.OBSERVE : State.SEARCH);
				}
			}
			case SEARCH -> {
				if (los) {
			...
			case OBSERVE -> {
				if (!los && stateTicks > 60) {
					enterState(State.SEARCH);
					return;
				}
				if (distance > PREFERRED_MAX_SQR) {
					enterState(State.POSITION);
					return;
				}
			...
			case POSITION -> {
				...
				if (distance <= PREFERRED_MAX_SQR && distance >= PREFERRED_MIN_SQR) {
					enterState(State.OBSERVE);
				} else if (distance > PREFERRED_MAX_SQR) {
					navigation.moveTo(target, 1.2D);
				} else if (repositionCooldown == 0 && stateTicks > 60) {
					enterState(State.REPOSITION);
				}
			}
			case ATTACK_PREP -> {
				navigation.stop();
				if (!los || distance > 3.5D * 3.5D) {
					enterState(State.POSITION);
					return;
				}
				if (stateTicks == 1 && level() instanceof ServerLevel serverLevel) {
					// A short, unmistakable telegraph makes the attack readable before
					// the impact frame instead of looking like instant damage.
					serverLevel.sendParticles(ModParticles.RESONANCE_ACTIVE, getX(), getEyeY(), getZ(),
							5, 0.12D, 0.12D, 0.12D, 0.01D);
				}
				if (stateTicks == 9) {
			...
			case RECOVER -> {
				navigation.stop();
				if (stateTicks >= 10) {
					enterState(getHealth() < getMaxHealth() * 0.3F ? State.RETREAT : State.OBSERVE);
				}
			}
			case REPOSITION -> {
				navigation.stop();
	...
	private void tickCombatPhase() {
		// Reversible enrage: the speed boost applies below half health and is
		// removed if it heals back up, instead of permanently mutating the
		...
		if (shouldEnrage && !enraged) {
			...
			getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(
					getAttribute(Attributes.MOVEMENT_SPEED).getBaseValue() + 0.10D);
			if (level() instanceof ServerLevel serverLevel) {
				serverLevel.sendParticles(ModParticles.VOID_STALKER_TRACE,
						getX(), getY() + 1.0D, getZ(), 16, 0.4D, 0.6D, 0.4D, 0.03D);
	...
	private <E extends ProductionVoidStalkerEntity> PlayState animation(AnimationState<E> animationState) {
		...
		// ATTACK_PREP plays the full anticipation -> impact -> recovery chain;
		// RECOVER continues the chain so the recovery animation is visible.
