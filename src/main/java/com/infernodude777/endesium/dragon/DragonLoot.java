 ...
 * Elytra bundle (4 Resonant Dragon Scales + supporting materials); later kills
 * grant reduced quantities and rolls. Looting never applies: the drops are
 * granted on the death hook, not through the vanilla loot table.
 */
public final class DragonLoot {
	...
	public static void grantDrops(EnderDragon dragon, ServerLevel level, boolean firstKill) {
		...
		if (firstKill) {
			...
			drop(dragon, ModItems.ENDESIUM_GUIDEBOOK, 1);
