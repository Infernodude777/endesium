...
/**
 * The vanilla Structure wrapper for one region's flagship. Placement is owned
 * entirely by the {@code endesium_flagships} random_spread structure set -
 * vanilla-style spacing, vanilla-style {@code /locate structure} and
 * {@code /place structure} support. When a piece generates, the build itself
 * is delegated to the original Feature code through a synthetic
 * {@link FeaturePlaceContext}, so every hand-authored block of every flagship
 * is byte-for-byte identical to the Feature-era builds. The wrapper only adds
 * a region gate (noise-biome sampled) and the anchor-chunk pattern that lets
 * one build span several chunks safely.
 */
public final class EndesiumFlagshipStructure extends Structure {
	...
	private static final int BOX_MARGIN = 26;
